...
### 📋 SBOM Reports
- Server and Frontend SBOM available in workflow artifacts
- Download artifacts from the Actions tab to access SBOM files
...
* try another changes ([5c9e684](https://github.com/batleforc/ProxyAuthK8S/commit/5c9e6844e3cde2d99d7e718b18a2f7cfd577ea86))
...
git checkout NR/release-v0.1.0

# Pull latest changes from remote
git pull origin NR/release-v0.1.0

# Merge latest changes from main (if needed)
git merge origin/main

# Push updated branch
git push origin NR/release-v0.1.0
```
...
find . -name "Cargo.toml" -not -path "./target/*" -exec sed -i 's/^version = ".*"/version = "0.1.0"/' {} \;

# Update frontend package.json files
find apps libs -name "package.json" -exec jq '.version = "0.1.0"' {} \; > tmp && mv tmp {}

# Commit version changes
git add .
git commit -m "chore: manual version bump to v0.1.0"
git push origin NR/release-v0.1.0
```
...
git push --force-with-lease origin NR/release-v0.1.0
```
...
git log origin/NR/release-v0.1.0..HEAD --oneline
```
...
All detailed reports and SBOM files are available as downloadable artifacts:
- 📊 **Quality Report**: Code quality analysis results
- 🔒 **Security Report**: Vulnerability audit findings
- ✅ **Commit Report**: Conventional commit validation
- 📋 **SBOM Reports**: Software Bill of Materials for Docker images
- 🐳 **Docker Security Report**: Container vulnerability scan results
- 📝 **Swagger Report**: API documentation validation
- 📚 **Changelog**: Generated changelog and release guide
